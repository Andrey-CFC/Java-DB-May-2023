create schema exam_prep_august_21;
use exam_prep_august_21;
...
where t.leader_id is not null and e.age < 40 and e.salary < 5000;
...
where g.name like '%2' and month(release_date) %2 = 0 and year(release_date) = 2022
order by quarter;
...
return (
select
concat_ws(' ', 'The', g.name,'is', 'developed', 'by', 'a', t.name, 'in', 'an', 'office', 
